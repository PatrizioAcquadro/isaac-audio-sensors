...
class StreamingLocalizer:
    ...
    def localize_block(self, samples, positions, rate, **kwargs):
        self.calls.append((samples.copy(), positions.copy(), rate, kwargs))
        assert not samples.flags.writeable
        assert not positions.flags.writeable
        return (), {"status": "unavailable", "count_status": "uncertain"}

    def reset(self):
        self.reset_count += 1


def test_streaming_localizer_receives_inactive_new_samples_and_explicit_orientation():
    localizer = StreamingLocalizer()
    pipeline = AudioPerceptionPipeline(
        activity_detector=FakeDetector(False), event_localizer=localizer
    )
    first = _block()
    second = replace(
        first,
        samples=first.samples * 2,
        time_window=AudioTimeWindow(start_time_s=2.0, end_time_s=3.0, frame_index=4),
    )
    frame = pipeline.process(
        first, _array(), frame_id="first", receiver_orientation_xyzw=(0, 0, 0, 2)
    )
    pipeline.process(second, _array(), frame_id="second")
    assert [c[0].shape for c in localizer.calls] == [(2, 4), (2, 4)]
    np.testing.assert_array_equal(localizer.calls[1][0], second.samples)
    assert localizer.calls[0][3] == dict(
        start_time_s=1.0, receiver_orientation_xyzw=(0, 0, 0, 1)
    )
    assert localizer.calls[1][3]["receiver_orientation_xyzw"] is None
    assert frame.diagnostics["perception"]["localization"]["status"] == "unavailable"
    assert frame.observations == ()
    pipeline.process(replace(second, discontinuity=True), _array(), frame_id="reset")
    assert localizer.reset_count == 1


def test_invalid_receiver_orientation_does_not_advance_stream():
    localizer = StreamingLocalizer()
    pipeline = AudioPerceptionPipeline(
        activity_detector=FakeDetector(False), event_localizer=localizer
    )
    with pytest.raises(ValueError):
        pipeline.process(
            ...
            _array(),
            frame_id="invalid",
            receiver_orientation_xyzw=(0, 0, 0, 0),
        )
    assert not localizer.calls
    assert pipeline._stream_end_s is None


def test_streaming_observation_survives_inactive_detector_and_trace_roundtrip():
    from isaac_audio_sensors.core.io.traces import (
        frame_from_trace_dict,
        frame_to_trace_dict,
    )

    class ActiveStreaming(StreamingLocalizer):
        def localize_block(self, samples, positions, rate, **kwargs):
            super().localize_block(samples, positions, rate, **kwargs)
            return (DoaEstimate(estimated_bearing_deg=25),), {
                "status": "events",
                "count_status": "uncertain",
            }

    pipeline = AudioPerceptionPipeline(
        activity_detector=FakeDetector(False), event_localizer=ActiveStreaming()
    )
    frame = pipeline.process(
        _block(), _array(), frame_id="observed", receiver_orientation_xyzw=(0, 0, 0, 1)
    )
    assert len(frame.observations) == 1
    assert not frame.diagnostics["perception"]["activity_detected"]
    restored = frame_from_trace_dict(frame_to_trace_dict(frame))
    assert restored.observations == frame.observations
    assert (
        restored.diagnostics["perception"]["localization"]["count_status"]
        == "uncertain"
    )
