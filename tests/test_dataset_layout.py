"""Acceptance tests for the frozen S2.1 session and shard layout."""
...
REFERENCE_DIR = Path("examples/datasets/reference_session_v1")
...
def test_manifest_marker_asset_correspondence_adversaries(tmp_path):
    ...
    # AssetRecord owns the layout-level kind/suffix primitive; S2.4 adds richer
    # multi-finding attribution around this same failure.
...
def test_rotation_layout_primitives_preserve_concatenation_model():
    ...
    # S2.2 owns renderer carry transfer.  At S2.1 the implementable primitive
    # is exact cut tiling: slicing once at cuts and concatenating introduces no
    # duplicate, silence, duration shift, or discontinuity.
