...
def test_render_meters_and_panel_compose_expected_shapes():
    ...
    meters = meter_view_models({"front": 0.5, "right": 0.01})
    bars = render_meters_rgba(meters, width=200, min_height=192)
    assert bars.shape == (192, 200, 4)
    assert bars.dtype == np.uint8
    # The louder mic's first bar row has a longer green fill.
    front_fill = int((bars[11, :, 1] > 150).sum())
    right_fill = int((bars[33, :, 1] > 150).sum())
    assert front_fill > right_fill > 0

    panel = render_instruments_panel_rgba(
        compass_view_model(bearing_deg=45.0, occluded=False), meters
    )
    assert panel.shape[0] == 192
    assert panel.shape[1] == 192 + 8 + 220
    assert panel.dtype == np.uint8


def test_write_rgba_png_emits_valid_png(tmp_path):
    import struct
    import zlib
    ...
    idat_len = struct.unpack(">I", data[data.index(b"IDAT") - 4 : data.index(b"IDAT")])[
        0
    ]
    raw = zlib.decompress(data[idat_start : idat_start + idat_len])
    assert len(raw) == 32 * (32 * 4 + 1)
