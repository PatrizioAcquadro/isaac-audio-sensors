...
- Breaking (Plan 02.1–02.2): changed `PropagationBackend` and capability output contracts from scene-to-frame `simulate()` to scene-to-signal `propagate()`. `AnalyticAcoustics` exposes the final effected mixture without DOA or waveform writes; its temporary scene-to-frame bridge now emits valid frame v3 with zero observations until Plan 02.3 removes it.
