...
- Integrated the registered `auditok` detector into maintained scalar runtimes. `simulate_from_config()` and CLI `simulate` now require an explicit runtime `energy_threshold_dbfs`; standard Isaac sensors require the same value, while callers that inject a custom `AudioPerceptionPipeline` must omit it. Active output is one `signal_derived` observation with detector ID `auditok`, no invented score, DOA, source identity, class, or source count.
- Breaking (Plan 03.3): advanced Kit configuration to `ias.omni_extension_binding.v6` with an exact required `activity_detection` object containing `detector_id="auditok"` and finite `energy_threshold_dbfs`; v5 and older inputs are rejected. The value is exposed by the UI, presets, headless summary, sensor session, and configuration round trip.
- Added reference-binding Auditok state per Isaac Lab environment with required `AudioArraySensorCfg.energy_threshold_dbfs`, selective reset, and no source-derived labels. The entity binding rejects that scalar-only field, and all six public Lab tensors remain unchanged and zero-filled until Phase 07.
- Reset live Isaac perception when the selected array's stream-defining layout changes, and aligned the live Isaac Sim and Kit gates with Auditok's 100 ms warm-up and legitimate post-reset empty frames.
...
- Added public `ObservationOrigin`, `AudioObservation`, and `AudioPerceptionPipeline` for injected activity detection, optional observed-only DOA, typed external observations, valid-channel filtering, block-derived aggregate RMS, deterministic ordering, and post-perception `max_observations` capping without source truth.
- Added public `ActivityDecision` and `ActivityDetector` contracts with stable plugin identity, required streaming reset, valid-channel NumPy input, and optional `[0, 1]` acoustic-activity probability. Detector-specific energy, threshold, and margin values remain diagnostics.
- Breaking (Plan 03.1): replaced the temporary activity callable and parallel pipeline `detector_id` argument with `ActivityDetector.detect() -> ActivityDecision`, added the validated `activity_detector` registry kind, and kept the default registry empty at that phase boundary pending detector qualification. Frame schema v3 remained unchanged.
- Added public fixed-threshold `AuditokActivityDetector` and the built-in `auditok` registry entry with causal bounded context, current-block token overlap, multichannel `any` semantics, deterministic reset, exact float32/dBFS adaptation for Auditok 0.5.2, and no synthetic activity probability. Initial calibration remains outside the streaming contract; consumer integration followed in Plan 03.3.
- Added Auditok as a Core dependency and as the sixth exact hash-locked Kit distribution, including bundled metadata, MIT license, artifact audits, and extracted-archive runtime validation without bundling NumPy.
- Breaking (Plan 02.2): replaced `AudioDetection` and `ias.audio_sensor_frame.v2` with the exact `ias.audio_sensor_frame.v3` observation contract. Frame fields now use `producer_id`, `channel_validity`, `max_observations`, and `observations`; v2 schemas, fixtures, parsing, configuration, and source-conditioned assembly were removed without aliases. Dataset-manifest and calibration-profile wrappers remain v1 and embed frame v3.
- Breaking (Plan 02.1–02.3): changed `PropagationBackend` and capability output contracts from scene-to-frame `simulate()` to scene-to-signal `propagate()`. `AnalyticAcoustics` exposes the final effected mixture without DOA, frame assembly, or waveform writes; the temporary scene-to-frame bridge, backend writer/cap arguments, and bridge-only assembly were removed without aliases.
- Added `core.simulation.simulate_frame()` as the single propagate-to-perception composition path, perception lifecycle reset, exact-block waveform and dataset recording, persistent Isaac signal-block ownership, direct guided recording, and per-environment Lab reference perception reset. At the Plan 02 boundary, default consumers remained zero-observation pending detector integration.
- Migrated Core, CLI, recording/replay, Replicator, Isaac, Lab, Kit, OmniGraph, examples, statistics, and live gates to observed-only names and semantics. Plan 02 intentionally produced waveform/RMS/frame output with zero observations and no oracle substitute; Plan 03.3 later integrated Auditok in maintained scalar consumers.
