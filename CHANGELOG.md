...
- Corrected Subphase 04.2 to qualify estimator roles independently: PyRoom SRP is the primary planar estimator at a 250 ms causal context and `0.06` reliability threshold; least-squares retains the distinct two-microphone ambiguity role; robustness remains failed and optional 3D remains blocked without invalidating the planar result. Internal SRP stays registered only until 04.3 cleanup, and NormMUSIC was not added or evaluated.
...
- Breaking (Plan 03): standard scalar runtimes now require `energy_threshold_dbfs`; Kit uses exact binding v6 with required `activity_detection`, while Isaac Lab reference binding owns one detector per environment and keeps all six public tensors zero-filled until Phase 07.
