...
- Mixture-only DOA plugins with PyRoom SRP qualified as the primary planar estimator for at least three non-collinear microphones and least-squares retained for honest two-microphone ambiguity; consumer selection remains explicit.
...
- `analytic_acoustics` supports Core free field and half space plus optional PyRoom shoebox and polygon prisms. Two-microphone least-squares reports both compatible azimuths instead of guessing, including both zero-TDOA bearings; its generic ambiguity semantics are qualified, while performance on any particular hardware remains consumer-owned. PyRoom SRP is the primary general planar estimator for at least three rank-2 XY microphones, with four non-collinear microphones recommended for practical redundancy. Robustness and consumer integration are not yet qualified. Optional 3D estimation remains available for downstream evaluation but has no representative real or realtime product evidence. Internal SRP remains temporarily registered pending Phase 04.3 cleanup. The mass-parallel Isaac Lab path is free-field and feature-only; scalar reference binding retains the honest two-microphone least-squares case, PyRoom, and closed-topology support. Occlusion attenuates only the direct source-to-microphone path; `surface_set`, diffraction, and reflected-path blocking remain outside this backend, and none of these controlled models is a complete wave solver or calibrated acoustic twin.
