...
- Explicit, default-off mixture-only DOA with PyRoom SRP for planar non-collinear arrays, least-squares for honest two-microphone ambiguity, an exact causal 250 ms window, and fail-closed temporal instability.
...
- `analytic_acoustics` supports Core free field and half space plus optional PyRoom shoebox and polygon prisms. Standard DOA is opt-in and never falls back between its two-microphone least-squares and planar PyRoom roles. Robustness under low SNR, occlusion, and low-level input is not qualified; particular two-microphone hardware remains consumer-owned; optional rank-3 use remains injection-only and unqualified. Isaac Lab reference mode projects observed activity and optional DOA into finite masked tensors; the mass-parallel entity path still emits no observations pending Subphase 07.2. Multiple slots do not imply qualified multisource perception. Occlusion attenuates only the direct path; `surface_set`, diffraction, and reflected-path blocking remain outside this backend.
