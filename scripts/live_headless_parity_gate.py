...
def main() -> int:
    ...
    for path in (gui_session, headless_session, gui_export, headless_export):
        _remove_tree(path)
    ...
    try:
        ...
        if gui_controller.guided_apply_preset("minimal_single_source") is None:
            raise RuntimeError("Could not apply the normalized parity preset.")
        if gui_controller.author_array(stage=stage) is None:
            raise RuntimeError("Could not author the normalized array.")
        if gui_controller.author_source(stage=stage) is None:
            raise RuntimeError("Could not author the normalized source.")
