#!/usr/bin/env python3
"""Replay the corrective package from its source commit and compare every byte."""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="ias-s4-7-corrective-02-replay-") as temp:
        ...
        completed = subprocess.run(
            [
                sys.executable,
                str(checkout / "scripts/generate_s4_7_corrective_02_evidence.py"),
                "--repo-root",
                str(checkout),
                "--output",
                str(replay),
    ...
    print(
        json.dumps(
            {
                "schema": "ias.s4_7.corrective_clean_replay.v3",
                "status": "passed",
