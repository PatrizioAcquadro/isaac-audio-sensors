#!/usr/bin/env python3
"""Replay S4.7 from its bound source commit and compare every evidence byte."""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="ias-s4-7-clean-replay-") as temp:
        ...
        command = [
            sys.executable,
            str(checkout / "scripts/generate_s4_7_evidence.py"),
            "--repo-root",
            str(checkout),
            "--output",
            str(replay),
    ...
    print(
        json.dumps(
            {
                "schema": "ias.s4_7.clean_replay_result.v1",
                "status": "passed",
