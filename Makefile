...
export-schema:
	PYTHONPATH=$(SOURCE_PYTHONPATH) $(PYTHON) -m isaac_audio_sensors export-schema --out $(SCHEMA_OUT)/audio_sensor_frame.v4.schema.json
	PYTHONPATH=$(SOURCE_PYTHONPATH) $(PYTHON) -m isaac_audio_sensors export-schema --schema dataset-manifest --out $(SCHEMA_OUT)/audio_dataset_manifest.v4.schema.json
