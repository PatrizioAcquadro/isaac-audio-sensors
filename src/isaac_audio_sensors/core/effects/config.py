    ...
    *,
    ...
    if ambient is not None:
        ...
        coherent = (
            ...
            else ambient.coherent_fraction
        )
    ...
    for mic_id, value in jitter_values:
        ...
        if sigma > 0.0:
            nonzero_stochastic = True
            if sample_count is not None and sample_count > 0 and math.ceil(
                6.0 * sigma * sample_rate_hz
            ) >= sample_count:
                raise ConfigValidationError(
    ...
    if config.enabled and backend_id in {"geometry_only", "tdoa_synthetic"} and (
        self_noise is not None or ambient is not None
    ):
...
def _validate_noise_level_spec(
    ...
    *,
    ...
    if not isinstance(config, NoiseLevelSpecConfig):
        raise ConfigValidationError(
            f"{table} must be NoiseLevelSpecConfig; received "
