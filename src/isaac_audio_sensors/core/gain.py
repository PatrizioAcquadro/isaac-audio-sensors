...
class GainValidationError(ValueError):
    """Invalid scalar amplitude-gain input or conversion result."""
...
def db_to_amplitude_gain(value: Real, field_name: str = "gain_db") -> float:
    ...
    if isinstance(value, bool) or not isinstance(value, Real):
        raise GainValidationError(
            f"{field_name} must be a real number, not a boolean."
        )
    gain_db = float(value)
    if not math.isfinite(gain_db):
        raise GainValidationError(f"{field_name} must be finite.")
    try:
