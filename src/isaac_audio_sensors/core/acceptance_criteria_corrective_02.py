"""Observation-bound evaluation for the additive S4.7 corrective_02 contract.
...
CONFIG_PATH = Path("configs/s4_7_holdout_acceptance.corrective_02.v3.json")
SCHEMA_PATH = Path(
...
PAYLOAD_SCHEMA = "ias.s4_7.corrective_metrics.v3"
RESULT_SCHEMA = "ias.s4_7.criteria_evaluation_result.v3"
...
_TDOA_FIELDS = frozenset(
    {"pair_id", "tdoa_us", "reference_tdoa_us", "absolute_error_us"}
)
_COMPARISON_FIELDS = frozenset({"comparison_id", "conditions"})
_CONDITION_FIELDS = frozenset(
    {"condition_id", "unadjusted_simulation", "adjusted_simulation"}
)
...
class TakeIdentity:
    ...
    target_bearing_deg_f_project: float | None
    duration_s: int
...
def load_corrective_config(repo_root: Path) -> dict[str, Any]:
    ...
    if config["schema"] != "ias.s4_7.holdout_acceptance_corrective_config.v3":
...
def build_identity_registry(
    ...
    for take_id, item in preliminary.items():
        ...
        registry[take_id] = TakeIdentity(
            ...
            target_bearing_deg_f_project=item["bearing"],
...
def evaluate_corrective(
    ...
    comparisons = _validate_comparisons(
        payload["sim_vs_real"], takes, registry, config
    )
        ...
        takes.append(
            {
                ...
                "candidate_bearings_deg_f_project": (
                    [float(identity.target_bearing_deg_f_project)]
                    if is_a or is_b
                    else []
                ),
                ...
                "conditions": [
                    {
                        ...
                        "unadjusted_simulation": _derive_real_condition_value(
                            entry, condition_id, take_map, registry, config
                        ),
                        "adjusted_simulation": _derive_real_condition_value(
                            entry, condition_id, take_map, registry, config
                        ),
                    }
...
def _validate_take_values(
    ...
    if not isinstance(failure_reasons, list) or any(
        ...
        raise CorrectiveAcceptanceError(
            f"{take_id}.failure_reasons must be a list of non-empty strings"
        )
    if len(set(failure_reasons)) != len(failure_reasons):
        raise CorrectiveAcceptanceError(f"{take_id}.failure_reasons has duplicates")
    if record["failed"] is not bool(failure_reasons):
        raise CorrectiveAcceptanceError(
            f"{take_id}.failed contradicts failure_reasons"
        )
    ...
    for channel in channels:
        ...
        if not isinstance(channel["sustained_clipping"], bool):
            raise CorrectiveAcceptanceError(
                f"{take_id}.{microphone}.sustained_clipping must be boolean"
            )
        derived_sustained = (
            channel["maximum_clip_run_samples"]
            >= config["clipping_contract"][
                "sustained_clipping_minimum_consecutive_samples"
            ]
        )
        if channel["sustained_clipping"] is not derived_sustained:
            raise CorrectiveAcceptanceError(
                f"{take_id}.{microphone}.sustained_clipping contradicts "
                "maximum_clip_run_samples"
            )
    ...
    reported_error = _optional_number(
    ...
    if bearing is not None and target is not None:
        derived_error = _circular_absolute_difference(target, bearing)
        if reported_error != derived_error:
            raise CorrectiveAcceptanceError(
                f"{take_id}.bearing_absolute_error_deg contradicts target and estimate"
            )
    sector_correct = _optional_bool(
        record["sector_correct"], f"{take_id}.sector_correct", required=is_b
    )
    if is_b:
        assert bearing is not None and target is not None
        derived_sector = (
            bearing_deg_to_sector_name(bearing)
            == bearing_deg_to_sector_name(target)
        )
        if sector_correct is not derived_sector:
            raise CorrectiveAcceptanceError(
                f"{take_id}.sector_correct contradicts target and estimate"
            )
    candidate_covered = _optional_bool(
        record["candidate_covered"],
        f"{take_id}.candidate_covered",
        required=is_a or is_b,
    )
    candidates = record["candidate_bearings_deg_f_project"]
    if not isinstance(candidates, list):
        raise CorrectiveAcceptanceError(
            f"{take_id}.candidate_bearings_deg_f_project must be a list"
        )
    if is_a or is_b:
        validated_candidates = [
            _number(
                item,
                f"{take_id}.candidate_bearings_deg_f_project",
                minimum=0.0,
                maximum=360.0,
                maximum_exclusive=True,
            )
        ...
        if len(set(validated_candidates)) != len(validated_candidates):
            raise CorrectiveAcceptanceError(
                f"{take_id}.candidate_bearings_deg_f_project has duplicates"
            )
        assert target is not None
        derived_candidate = any(
            _circular_absolute_difference(target, item) <= 20.0
            for item in validated_candidates
        )
        if candidate_covered is not derived_candidate:
            raise CorrectiveAcceptanceError(
                f"{take_id}.candidate_covered contradicts candidate bearings"
            )
    elif candidates:
        raise CorrectiveAcceptanceError(
            f"{take_id}.candidate_bearings_deg_f_project is not applicable"
        )
    _optional_number(
        record["confidence"],
    ...
    audio_time = _optional_number(
        record["audio_event_time_ms"],
        f"{take_id}.audio_event_time_ms",
        required=is_e,
    )
    video_time = _optional_number(
        record["video_event_time_ms"],
        f"{take_id}.video_event_time_ms",
        required=is_e,
    )
    av_residual = _optional_number(
    ...
    if is_e:
        assert audio_time is not None and video_time is not None
        if av_residual != abs(audio_time - video_time):
            raise CorrectiveAcceptanceError(
                f"{take_id}.av_absolute_residual_ms contradicts event times"
            )
...
def _validate_tdoa(
    ...
    for item in value:
        ...
        reference = _number(
            item["reference_tdoa_us"],
            f"{take_id}.{pair_id}.reference_tdoa_us",
            minimum=domain["minimum"],
            maximum=domain["maximum"],
        )
        reported_error = _number(
            item["absolute_error_us"],
            f"{take_id}.{pair_id}.absolute_error_us",
            minimum=0.0,
        )
        if reported_error != abs(measured - reference):
            raise CorrectiveAcceptanceError(
                f"{take_id}.{pair_id}.absolute_error_us contradicts "
                "measured and reference TDOA"
            )
    if seen != expected:
        raise CorrectiveAcceptanceError(
    ...
    for index, record in enumerate(value):
        ...
        for condition in conditions:
            ...
            values = {
                ...
                for path in config["sim_vs_real"]["payload_paths"]
            }
            values["real"] = _derive_real_condition_value(
                entry, condition_id, takes, registry, config
            )
        ...
        aggregates = {
            ...
            for path in config["sim_vs_real"]["reported_paths"]
        }
        raw_change = (
...
def _derive_real_condition_value(
    entry: Mapping[str, Any],
    condition_id: str,
    takes: Mapping[str, Mapping[str, Any]],
    registry: Mapping[str, TakeIdentity],
    ...
    """Derive the real path from the matching authenticated take observation."""

    comparison_id = entry["comparison_id"]
    if entry["condition_kind"] == "take_microphone_pair":
        try:
            take_id, pair_id = condition_id.rsplit("|", 1)
        except ValueError as exc:  # pragma: no cover - identity set checks first
            raise CorrectiveAcceptanceError(
                f"{comparison_id} invalid take-pair condition: {condition_id}"
            ) from exc
        take = takes.get(take_id)
        if take is None:
            raise CorrectiveAcceptanceError(
                f"{comparison_id} unknown take condition: {take_id}"
            )
        records = {
            item["pair_id"]: item
        ...
        if pair_id not in records:
            raise CorrectiveAcceptanceError(
                f"{comparison_id} unknown microphone pair: {condition_id}"
            )
        value: Any = records[pair_id]["absolute_error_us"]
    else:
        take_id = condition_id
        take = takes.get(take_id)
        identity = registry.get(take_id)
        if take is None or identity is None:
            raise CorrectiveAcceptanceError(
                f"{comparison_id} unknown take condition: {take_id}"
            )
        if comparison_id == "bearing_doa_error_ab":
            value = take["bearing_absolute_error_deg"]
        elif comparison_id == "sector_accuracy_b":
            value = 1 if take["sector_correct"] is True else 0
        elif comparison_id == "candidate_bearing_ab":
            value = 1 if take["candidate_covered"] is True else 0
        elif comparison_id == "abstention_abd":
            windows = take["window_summary"]
            source = windows["source_window_count"]
            abstained = windows["abstained_window_count"]
            value = (
                source - abstained
                if identity.stratum_id == "D_silence"
                else abstained
            ) / source
        elif comparison_id == "confidence_bc":
            value = take["confidence"]
        elif comparison_id == "coarse_audio_video_association_e":
            value = take["av_absolute_residual_ms"]
        else:
            raise CorrectiveAcceptanceError(
                f"unsupported real-value derivation: {comparison_id}"
            )
    return _comparison_number(
        value,
        f"{comparison_id}.{condition_id}.derived_real",
        entry,
    )
...
def _comparison_number(value: Any, label: str, entry: Mapping[str, Any]) -> float:
    ...
    if unit in {"fraction", "dimensionless"}:
        number = _number(value, label, minimum=0.0, maximum=1.0)
        if entry["comparison_id"] in {
            "sector_accuracy_b",
            "candidate_bearing_ab",
        } and number not in {0.0, 1.0}:
            raise CorrectiveAcceptanceError(f"{label} must be exactly 0 or 1")
        return number
