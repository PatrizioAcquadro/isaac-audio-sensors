...
def render_meters_rgba(
    meters: Sequence[MeterViewModel],
    *,
    width: int = 220,
    row_height: int = 22,
    min_height: int = COMPASS_IMAGE_SIZE,
) -> np.ndarray:
    """Rasterize per-mic meter bars (fill fraction only; labels live in UI)."""

    width = int(width)
    height = max(int(min_height), len(meters) * int(row_height))
    rgba = np.zeros((height, width, 4), dtype=np.uint8)
    rgba[..., :3] = 30
    rgba[..., 3] = 255
    pad = 6
    bar_height = int(row_height) - 2 * pad
    for index, meter in enumerate(meters):
        top = index * int(row_height) + pad
        bottom = top + max(bar_height, 4)
        rgba[top:bottom, pad : width - pad, :3] = 55
        fill_width = int(round(meter.fraction * (width - 2 * pad)))
        if fill_width > 0:
            color = COLOR_OCCLUDED if meter.fraction >= 0.98 else COLOR_CLEAR
            channel = np.array(
                [int(color[0] * 255), int(color[1] * 255), int(color[2] * 255)],
                dtype=np.uint8,
            )
            rgba[top:bottom, pad : pad + fill_width, :3] = channel
    return rgba


def render_instruments_panel_rgba(
    view_model: CompassViewModel,
    meters: Sequence[MeterViewModel],
    *,
    compass_size: int = COMPASS_IMAGE_SIZE,
    gap: int = 8,
) -> np.ndarray:
    """Compose the compass and meter rasters into one panel image."""

    compass = render_compass_rgba(view_model, size=compass_size)
    bars = render_meters_rgba(meters, min_height=compass_size)
    height = max(compass.shape[0], bars.shape[0])
    width = compass.shape[1] + int(gap) + bars.shape[1]
    panel = np.zeros((height, width, 4), dtype=np.uint8)
    panel[..., :3] = 30
    panel[..., 3] = 255
    panel[: compass.shape[0], : compass.shape[1]] = compass
    offset = compass.shape[1] + int(gap)
    panel[: bars.shape[0], offset : offset + bars.shape[1]] = bars
    return panel


def write_rgba_png(path: Any, rgba: np.ndarray) -> Any:
    """Write an RGBA uint8 array as a PNG using only the stdlib."""
