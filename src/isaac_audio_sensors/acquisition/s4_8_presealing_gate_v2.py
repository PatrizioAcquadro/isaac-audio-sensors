    ...
    if (
        ...
        if waveform is not None:
            ...
            if any(
                ...
                and item["relative_correlation_median"]
                <= gate["maximum_negative_reference_correlation"]
...
def detect_tracked_reference_activity_v2(
    ...
    *,
    ...
    for index, start in enumerate(
        ...
        if tracking_correlation < float(detector["minimum_reference_correlation"]):
            ...
            if wide_correlation >= float(
                ...
                local_reasons.append("reference_tracking_weak")
        if "phase_discontinuity" in local_reasons:
            alignment_maintained = False
            if "phase_discontinuity" not in alignment_failure_reasons:
                alignment_failure_reasons.append("phase_discontinuity")
        else:
        ...
        if tracked_adjustment != adjustment:
            channel_evidence = [
                _lag_polarity_evidence(
                    channel,
                    reference_frame,
        ...
        relative_polarity_correlations = [
            float(
                median(
                    _best_signed_correlation(
                        frame[channel],
                    ...
                    for peer in range(frame.shape[0])
                    if peer != channel
                )
            )
        ...
        reference_correlation = float(np.median(correlation_magnitudes))
        ...
        decisions.append(
            {
                ...
                "relative_polarity_correlation_by_channel": (
                    relative_polarity_correlations
                ),
                "reference_lag_samples_by_channel": reference_lags,
    ...
    # A single weak reference block between two valid blocks is detector
    # uncertainty, not evidence that continuous playback stopped.  Energy and
    # multichannel coherence still have to pass in the weak block.
    stabilized_candidates = list(raw_candidates)
    for index in range(1, len(raw_candidates) - 1):
        weak_reference_only = set(reason_sets[index]).issubset(
            {"reference_tracking_weak", "reference_correlation"}
        )
        if (
            not raw_candidates[index]
            and raw_candidates[index - 1]
            and raw_candidates[index + 1]
            and weak_reference_only
        ):
            stabilized_candidates[index] = True
    ...
    if any(
        ...
        alignment_maintained = False
        alignment_failure_reasons.append("reference_alignment_lost")

    useful = [False] * len(stabilized_candidates)
    useful_runs: list[tuple[int, int]] = []
    for start, stop in _boolean_runs(stabilized_candidates):
    ...
    channel_polarity_evidence = []
    for channel in range(mic.shape[0]):
        positive_median = float(
            median(
                item["positive_reference_correlation_by_channel"][channel]
        ...
        relative_median = float(
            median(
                item["relative_polarity_correlation_by_channel"][channel]
                for item in decisions
            )
        )
        channel_polarity_evidence.append(
            {
                "channel_index": channel,
                "status": _polarity_status(
                    max(0.0, relative_median),
                    max(0.0, -relative_median),
                    minimum_correlation=float(
                        detector["minimum_reference_correlation"]
                    ),
                ),
                "positive_correlation_median": positive_median,
                "negative_correlation_magnitude_median": negative_median,
                "signed_correlation_median": signed_median,
                "relative_correlation_median": relative_median,
            }
        )
    ...
    return {
        ...
        "candidate_block_count": sum(stabilized_candidates),
        ...
        "channel_polarity_evidence": channel_polarity_evidence,
...
def _evaluate_stop_sentinel(
    ...
    *,
    ...
    present = (
        tracking_correlation >= float(config["minimum_reference_correlation"])
...
def _best_common_phase_adjustment(
    ...
    *,
    ...
    """Search for a polarity-independent common phase after tracking is lost."""
    ...
    for adjustment in range(center_adjustment - radius, center_adjustment + radius + 1):
        ...
        correlation = abs(_signed_correlation(frame[0], candidate))
...
def _track_common_phase_step(
    ...
    *,
    ...
    """Track common timing from polarity-independent per-channel lag evidence."""

    current_reference = _reference_frame(
        reference,
        phase_start=nominal_phase + center_adjustment,
        sample_count=frame.shape[1],
    )
    current_evidence = [
        _lag_polarity_evidence(
    ...
    requested_step = int(
        round(median(item["lag_samples"] for item in current_evidence))
    )
    step = max(
        -maximum_step_samples,
        min(maximum_step_samples, requested_step),
    )
    adjustment = center_adjustment + step
    adjusted_reference = _reference_frame(
        reference,
        phase_start=nominal_phase + adjustment,
        sample_count=frame.shape[1],
    )
...
def _lag_polarity_evidence(
    left: np.ndarray,
    right: np.ndarray,
    maximum_lag_samples: int,
) -> dict[str, float | int]:
    """Measure lag, presence magnitude, and polarity without conflating them."""
    ...
    for lag in range(-maximum_lag_samples, maximum_lag_samples + 1):
        if lag < 0:
            ...
            x = left
            y = right
        correlations.append((_signed_correlation(x, y), lag))
    signed_correlation, lag = max(
        correlations,
        key=lambda item: (
            abs(item[0]),
            -abs(item[1]),
    ...
    positive_correlation = max(
        0.0,
        max(item[0] for item in correlations),
    )
    negative_correlation_magnitude = max(
        0.0,
        -min(item[0] for item in correlations),
    )
    return {
        "lag_samples": lag,
...
def _polarity_status(
    ...
    *,
    ...
    positive = positive_correlation >= minimum_correlation
    negative = negative_correlation_magnitude >= minimum_correlation
