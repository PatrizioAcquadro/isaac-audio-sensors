...
- Post-release, unreleased: added array-listener reuse with a session-layer fallback plus manual qualitative Kit mix capture with verified WAV metadata, safe active-listener restoration, and lifecycle cleanup; Sensor WAV playback and microphone-array observations remain separate.
- Post-release, unreleased: completed native finite/infinite `loopCount` authoring and discovery, rejected non-spatial sounds from physical-sensor discovery, and exposed source loop count in the Kit UI/configuration.
