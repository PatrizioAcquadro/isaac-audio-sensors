...
`docs/reference_rig_hardware_environment.md`, and the future printed-mount
design handoff `docs/zed_respeaker_mount_model_handoff.md`. Revision A Option 1 is digitally
...
handmade `S4_TEMP_DESKTOP_FIXTURE_REV0` in WANG 2022: ZED below, ReSpeaker above
on two operator-fixed inverted plastic supports over a corrugated-cardboard
riser, with operator-reported approximate
...
| Gate | Required inputs | Measurable outputs and evidence | Pass criteria | Failure handling |
| --- | --- | --- | --- | --- |
| `S4.1` Rig, mount, geometry, and frame lock | S0.5 facts and current eight-gate ledger; S1.2 profile schema; hardware/environment record; actual installed-mount record; future printed-mount handoff for boundary context only. | Versioned rig/device/channel/frame/source/room/topology record, actual handmade assembly state, practical mount-check record, evidence-status ledger, approximate as-used geometry, uncertainty/limitations, hashes, and any available future-CAD reference kept explicitly separate at `outputs/isaac_audio_sensors/S4/S4.1/`; closeout `docs/development/closeouts/S4/s4_1_bom_frame_lock.md`. | Independent review reproduces channel order, coordinate conventions, placement method, and functionally required as-used geometry. The installed rig is stable, does not move unintentionally, is repeatable enough for target metrics, leaves microphone openings/FOV sufficiently clear, and has safe non-disturbing cables. Future CAD geometry is never assigned to the installed handmade fixture or called measured extrinsic; unmeasured values block only claims that require them. | Unsafe/unstable/obstructed setup or missing required identity/frame facts is fixed or blocked. Unavailable files for a future unbuilt mount do not block the different installed fixture. Formal companion-CAD fabrication and physical-qualification gates remain separate and are not falsely passed or automatic S4 blockers. |
