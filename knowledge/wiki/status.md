...
On 2026-09-09 the user reopened reliable joint event count and direction over time and requested the following work **before 07.2**. This supersedes the earlier decision to pause motion-perception research and proceed with the bounded reference. The temporal and live-occlusion outcomes remain open; confidence availability is now implemented in frame v4:

- [[implementation_phases/04-observed-direction-estimation#Pre-07.2 Follow-up — Joint Count and Direction over Time|Temporal perception]]: useful count and directions together during onset, offset, motion, overlap and partial occlusion, preserving stationary/indoor usefulness and explicit limits. If a useful supported domain cannot be achieved, return the scope decision to the user rather than silently advancing 07.2.
- [[implementation_phases/02-signal-and-perception-architecture|Confidence availability]]: frame v4 distinguishes unavailable (`None`) from measured zero through observations, replay, GUI and masked learning/Lab projection; prior frame versions are rejected.
- [[implementation_phases/r8-analytic-acoustics-backend#Pre-07.2 Follow-up — Live Occlusion Correctness|Live occlusion correctness]]: support ordinary solid obstacles and make requested-but-unavailable occlusion explicit.

Then proceed through [[implementation_phases/07-isaac-lab-observation-integration|07.2 stateful/scalable integration and 07.3 observed GUI/consumer consolidation]], [[implementation_phases/08-geometry-acoustics-integration|08 geometry acoustics]], and [[implementation_phases/09-practical-realism-and-randomization|09 evidence-backed realism]]. Full geometry acoustics and every possible effect are not pre-07.2 requirements. [[topics/onr-video-production#Video 4 Readiness and Scope|ONR Video 4]] distinguishes the earlier direct-attenuation opportunity from the fuller geometry version after 08.3.

Confidence migration passes 645 unit/contract tests, 394 integration/release tests, and 18 supported-runtime tensor tests including actual RTX 4090 projection. Full live lifecycle checks remain part of the occlusion closeout.

The local audit at `babda6f` exercised actual PhysX/Kit on the RTX 4090, 320 audio windows and CUDA observation projection. Direct attenuation, source summation and static band-filter continuity pass the bounded controls. Solid colliders can fail capture, unavailable confidence is projected as valid zero, and the GUI hides simultaneous-event bearings and unselected ambiguity candidates. Controlled unavailable-raycaster output can still appear ACTIVE. Partial attenuation and some clear single-source controls expose perceptual errors. The evidence in `local/occlusion_audit/` is a bounded synthetic assessment, not physical/material qualification, realtime performance proof, or a full visual/manual GUI review. The named phase pages own the corrections; no SDK change is delivered by this roadmap update.
...
Fast indoor multisource perception remains **unresolved: NO-GO for replacing the localizer**. A joint-count/direction investigation now compares causal spatial memory, recursive DP-RTF and complete ODAS SSL/SST through the shared pipeline. The strongest temporal control improves one square-array moving case from 33.3% to 76.7% joint success after common warm-up, but reaches only 17.2–42.3% on the other three geometries. A bounded 65.6 s real LOCATA comparison also remains inadequate. The original 750 ms WPE/group-sparse algorithm and corrected propagation are preserved; original moving-pair results reproduce exactly. An optional streaming injection interface and received-audio numerical evaluator are delivered, not a qualified new perceptual algorithm. Independent confirmations were not consumed after nominal screening failed. See [[experiments/04-4-multisource-localization|the joint-motion investigation, limits and next acoustic-front-end experiment]]. That investigation included no 07.2, video, demonstration scenes or ONR production.

**Superseded sequencing decision — motion-perception pause (2026-09-09).** The user initially paused the unsuccessful improvement iteration while retaining the localizer, allowing bounded 07.2 work to proceed. The later Current Priorities above reopen temporal count/direction as a pre-07.2 objective. Neither decision changes the unresolved numerical results or qualifies a replacement algorithm. Video 3 subsequently completed its separately bounded production check; that scene-specific outcome does not establish general moving-source perception.
...
- `analytic_acoustics` does not accept `surface_set`. Isaac Lab reference mode projects scalar observed activity and optional DOA. Entity mode still returns empty observations; scalable waveform/features and per-environment acoustic variation remain unqualified. Bounded scalar multisource sensing at 16 kHz is qualified by 04.4; arbitrary reverberant mixtures are not. Policy tensors contain no waveform, mixture RMS, SPL, or calibration values.
...
Subphases 04.1–04.3, Plans [[implementation_phases/05-ground-truth-and-learning-datasets|05]] and [[implementation_phases/06-simulated-and-real-signal-parity|06]], and [[implementation_phases/07-isaac-lab-observation-integration|07.1 Lab tensor projection]] are complete. The requested [[implementation_phases/04-observed-direction-estimation|04.4 indoor-improvement intervention]] is now confirmed and integrated: one WPE/group-sparse covariance path passes all 24 joint indoor geometry/condition quality gates on two fresh blocks, plus common consumers and actual RTX 4090 Lab projection. **This is bounded simulated stable-source usefulness, not complete 04.4 or physical/general indoor qualification. 07.2 has not started.** Stereo and other planar rates retain their previous single-event role. Weak speech remains imperfect, changes can take 1–1.5 s, 3D compute exceeds 50 ms, and original planar direct-path operational pair criteria regress. The [[experiments/04-4-multisource-localization|experiment]] owns exact before/after results and limitations. No tracking, separation, known-count input or two-source cap was added. Broader and physical validation, and future scalable Lab work, remain separate next steps; this intervention does not authorize or start them. The selected Geometry Acoustics provider and [[implementation_phases/09-practical-realism-and-randomization|Plan 09]] remain planned; historical gain corrections remain rejected/inconclusive.
...
the completed bounded 04.4 reference and the reopened temporal objective before 07.2; the other plans retain
