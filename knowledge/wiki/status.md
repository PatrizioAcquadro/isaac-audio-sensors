...
**Project decision — motion-perception research paused (2026-09-09).** Close the current improvement iteration and retain the maintained localizer; reliable joint count/direction perception during ordinary motion remains unresolved. Further acoustic experiments are deferred, not a prerequisite for beginning integration of the qualified bounded reference and a separately validated scalable path in [[implementation_phases/07-isaac-lab-observation-integration|07.2]]. That phase must preserve and characterize the reference's errors, latency and availability rather than substitute source truth or claim to repair perception. Video 03 may return to a focused feasibility check of approach/pass/recede through its actual sensor, perception and recording path before production. Corrected propagation alone does not qualify its direction display or downstream reactions. This documentation decision starts neither activity and grants no new moving-perception qualification.

Video 03 has passed a bounded natural-engine, single-source GPU articulation and received-audio gate. Following user approval of the continuous-engine audition and requested timing changes, the current pass lasts 23 s at 8 m/s with 12 m clearance. The scored 8–17 s spatial interval has complete direction availability, p95 bearing errors below 4.4 degrees and 100% camera membership after the acquisition allowance. All 460 windows replay sample-identically. Numerical Doppler, travel delay and geometric level checks pass. Outdoor asset inspection and the required excerpt review remain incomplete; no editorial video has been produced. See [[topics/onr-video-production|ONR Video Production]] for distant-interval exclusions, listening status and evidence.
