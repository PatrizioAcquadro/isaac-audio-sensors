...
Fast indoor multisource DOA remains **unresolved: NO-GO for replacing the localizer**. Adaptive weighting, equal-count composition, per-direction refinement and an established SRP refinement were compared on corrected samples. Faster direction updates help isolated motion but do not provide reliable improvement on separated moving speech pairs with receiver translation/rotation. The final conservative control has 18–40 exact updates of 66 per geometry versus 19–42 for the baseline. The SDK therefore retains its original 750 ms WPE/group-sparse implementation unchanged. See [[experiments/04-4-multisource-localization|the motion closeout, measured regressions and reusable numerical evaluator]]. No 07.2, video, demonstration scene or ONR production was started.
