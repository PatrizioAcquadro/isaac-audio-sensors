...
- Subphases 04.1–04.2 lock `DoaEstimator` to ordered mixture samples, matching array-local XYZ positions, and sample rate, then qualify lazy optional `pyroomacoustics_srp` independently as the primary general planar estimator for at least three non-collinear microphones. Its qualified causal operating point uses a 250 ms observation block, 512-point STFT, observed-energy bins within 300–6000 Hz, 2-degree planar grid, and estimator-local reliability threshold `0.06`. Least-squares is separately qualified for physically honest two-microphone ambiguity, including zero TDOA; internal SRP remains unqualified until 04.3 removal. The pipeline still exposes no scene, source truth, private stems, or producer diagnostics, and no maintained consumer selects a DOA estimator before 04.3.
