...
- Subphases 04.1–04.3 retain the exact mixture-only `DoaEstimator` contract and integrate one explicit standard selector: two microphones route only to `tdoa_least_squares`, horizontal rank-2 XY arrays route only to lazy `pyroomacoustics_srp`, and rank-3 arrays remain injection-only and unqualified. The consumer owns an exact trailing causal 250 ms window plus fail-closed 150-degree jump / 30-degree confirmation handling. Internal SRP and the completed qualification harness are removed, while ignored reports remain local historical evidence; no estimator fallback exists.
...
- Maintained scalar Core, CLI, Isaac Sim, Kit, and Isaac Lab reference entry points resolve Auditok only with an explicit runtime threshold and expose DOA as `doa_enabled=False` / `--enable-doa`. Active output is at most one observed-only `signal_derived` observation; DOA warm-up returns unresolved `insufficient_context`. Custom pipelines remain caller-owned and cannot be combined with standard DOA activation. `AudioSensorConfig`, TOML, frame schema v3, `simulate_frame()`, `DoaEstimate`, and direct `AudioPerceptionPipeline` composition remain unchanged.
...
Subphases 04.1–04.3, Plans [[implementation_phases/05-ground-truth-and-learning-datasets|05]] and [[implementation_phases/06-simulated-and-real-signal-parity|06]], and [[implementation_phases/07-isaac-lab-observation-integration|07.1 Lab tensor projection]] are complete. [[implementation_phases/04-observed-direction-estimation|04.4]] is partially implemented: isolated candidate comparison and independent simulated evaluation return **NO-GO for both planar and 3D integration**. The strongest correction passes the latest independent static, idle and transition gates on all four layouts, but broader known-case regression still fails pair counting on triangle, square and tetrahedron. See the [[experiments/04-4-multisource-localization|qualification results]]. Next, compare a dedicated event-cardinality approach against the retained baseline, then use broader fresh independent content before common-perception integration and the new Lab GPU smoke. Longer context did not repair the observed failures. It advances localization from 11.3, without advancing tracking or audio separation. Two sources are the first validation milestone, not a structural limit. The selected Geometry Acoustics provider remains separate planned work. [[implementation_phases/09-practical-realism-and-randomization|Plan 09]] remains planned: the physical comparison prioritizes received level, ambient interference, coherent channel variation, and activity near threshold before expensive acoustic detail. Historical gain decisions remain rejected/inconclusive; no correction is enabled.
