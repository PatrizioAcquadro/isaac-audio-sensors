...
## 2026-08-20 — closure: R3 residual product boundary

Recorded generic built-in rig profiles, removal of the external showcase
fixture from the Kit smoke, stricter release policy, and final host, GPU,
consumer, and evidence validation.

## 2026-08-20 — update: R2-R3 test and boundary cleanup

Recorded the semantic test system, public package boundary, release policy,
verified lanes, and retained-evidence boundary from the current checkout.

## 2026-08-20 — lint: Canonical wiki

Verified the allowed tree, raw-file preservation, internal links, index
coverage, current claims, and Markdown whitespace.

## 2026-08-20 — update: R4 documentation consolidation

Replaced the root documentation collection with current canonical topic, decision, phase, status, and navigation pages; retained the R0 specification byte-for-byte in its authorized raw location.

Reduced the root README and standalone Kit metadata, removed stale GUI screenshots and historical duplication, and added the tested documentation boundary.
...
## 2026-09-10 — update: Close 07.2 with practical baseline measurements

The user confirms 4096 copies were exploratory, retains the moderate-indoor objective and defers all component comparisons. Measure 2/16/32/64/128/256 planar and raised batches with unchanged float64 WPE, 750 ms context, 16 kHz PCM, 60 Hz acquisition and 10 Hz observations. Every accepted run preserves scalar counts on identical final PCM (996/996 comparisons), with false events and maximum directional deviations reported. An added one-degree maximum diagnostic proved stricter than the established paired criterion; retain its failure and use the existing 97% count / five-degree p95 preservation rule, without changing the localizer. Repeat the variable planar runs after host checks finish. Recommend 16 copies for interactive audio development and 128 as an initial collection batch: about 250/472 ms per planar/raised update, versus 462/936 ms at 256. Record memory, separate reset costs and waiting-time extrapolations. Close bounded 07.2 without claiming full robot-training throughput, realtime 4096-copy operation or scalable indoor propagation. 07.3 remains the next implementation phase.
