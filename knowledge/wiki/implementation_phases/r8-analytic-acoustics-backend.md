...
At the R8.2 boundary, `SourceOcclusion` required `array_id`, `source_id`, exact `per_mic_blocked` and `per_mic_attenuation_db` maps, and `occlusion_model`; optional aligned band attenuation, band centers, per-microphone hit paths, and hit-material provenance remained. Aggregate `occlusion_factor`, `attenuation_db`, and `hit_prim_paths` fields and duplicate applied-gain diagnostics were removed without aliases. R8.4 later removed the remaining model, hit-path, and material fields from this Core record. `AudioDetection.occluded` and the concise UI `occlusion_factor` diagnostic remain derived from `per_mic_blocked`.
...
R8.4 completes the analytic occlusion cleanup after R8.3 stabilized the scalable backend and migrated the legacy consumers. `SourceOcclusion` is limited to `array_id`, `source_id`, exact `per_mic_blocked` and `per_mic_attenuation_db` maps, plus optional aligned band attenuation and band centers. `per_mic_hit_prim_paths`, `hit_materials`, and per-record `occlusion_model` were removed without aliases. The producing model, partition-level material evidence, and every unknown-material fallback application are reported once under frame-level `acoustics_state`; pure-Core callers receive no invented provenance.

Hit-path persistence is replaced by an optional Isaac-internal trace. The normal path emits only the minimal `SourceOcclusion`; existing `debug_draw` additionally captures source and microphone endpoints, hit points, prim paths, acoustic-partition ids, resolved materials, applied losses, and deduplication state. `DebugPrimitive` ray and hit records feed the existing live draw, JSON sidecar, USD debug-geometry, and review-capture paths. The trace is absent from `AudioSceneSnapshot`, the stable `AudioSensorFrame` schema, and ordinary datasets, and it is not allocated when debug capture is disabled.

Accumulation is defined over acoustic partitions rather than arbitrary visual or collision prims. Optional `ias:acoustic_partition_id` groups multiple prims under one authored assembly curve; a missing attribute uses the collider prim path as its implicit partition. Fragmentation does not duplicate loss, conflicting curves fail closed, distinct sequential partitions add in dB, and exceeding `max_hits_per_ray` fails instead of returning truncated attenuation. Alternative direct, reflected, diffracted, or around-opening paths remain separate waveform contributions and are never represented by summing obstacle losses. The analytic model accepts an authored whole-assembly curve for constructions such as double-leaf walls and does not implement structural wall physics.

The fixed `60 dB` total-loss clamp and `occlusion_attenuation_cap_db` are removed. `max_hits_per_ray` remains only as a fail-closed bounded-work guard. `occlusion_max_attenuation_db` is replaced by `unknown_material_loss_db`, used only as an explicit nominal fallback for unresolved materials. Authored broadband and band losses pass through unchanged, including sequential totals above 60 dB.
...
R8.3 adds deterministic host coverage, 90 Isaac-runtime tests, optional PyRoom/SciPy/SoundFile execution, and RTX 4090 live gates. Isaac Sim passes three analytic lifecycle frames; Isaac Lab passes entity/reference parity, partial reset, CUDA tensor contracts, and 50 steps over 4096 environments at 2.213 ms/step mean against the 20 ms budget; Kit passes all 38 workflow steps with binding v4, a four-channel analytic waveform, and a non-silent qualitative device mix. The migrated SquadBot suite passes 373 tests with 10 expected skips and no removed-API failures. Clean-source `3.0.0` sdist, universal wheel, and Kit ZIP pass their local audit. The three serialized v1 schemas and authenticated downstream artifacts remain unchanged.

R8.4 passes 460 host tests, 200 integration tests, 57 release tests, and 100 tests through the supported Isaac runtime, plus configuration, fixture, Ruff, and optional PyRoom/SciPy/SoundFile gates. RTX 4090 Isaac Sim evidence records a blocked-to-clear factor transition from `1.0` to `0.0`, a hit-bearing debug trace before movement, a clear ray afterward, and geometry-free detection diagnostics. Isaac Lab preserves entity/reference parity and partial reset across 50 steps and 4096 environments at 2.241 ms/step mean against the 20 ms budget. Kit passes all 38 workflow steps with four-channel waveform output and a readable non-silent qualitative device mix. The unchanged SquadBot checkout passes 373 tests with 10 expected skips. A clean clone of implementation commit `33812b6` passes preflight and the complete `3.0.0` sdist, universal-wheel, and Kit-ZIP audit. All three v1 schemas regenerate byte-identically.
...
- `src/isaac_audio_sensors/isaac/viz/`
...
- 2026-09-01: Implemented R8.4 minimal Core occlusion records, partition-based uncapped Isaac transmission, frame-level provenance, optional transient debug traces, and unchanged v1 schemas.
