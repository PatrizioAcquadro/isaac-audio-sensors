...
**Current outcome: GO for the bounded planar and 3D scalar reference; indoor robustness remains incomplete and takes priority before 07.2.** The user-directed integration milestone separates useful reference behavior from the original broader acoustic target. The unchanged weighted covariance-AIC candidate passes the prospective direct-path protocol on 1,728 new cases using 16 previously unused speakers, plus idle, response and known-case regression within that domain. Combined room/imbalance and other difficult cases remain measured limitations, not retrospectively successful qualification. See the experiment for the fixed domain and results.
