...
Status: investigated on 2026-09-09; **NO-GO for a temporal replacement, 07.2 blocked**. Persistent OnlineWPE, normalized spatial evidence, their combination and matched-history controls fail the ordinary moving-speech screen and can regress useful static controls. No candidate is integrated. The [[experiments/04-4-multisource-localization#Pre-07.2 Acoustic Front-End Evaluation|new front-end evaluation]] records the domain, paired results, counterexamples and unmet confirmation gate. The bounded stable-source implementation is preserved; it does not complete the renewed objective. A user scope decision is required before further work or any change to this gate.
