...
The sensor has two binding modes. `bind_entities(scene, cfg)` is the vectorized training path and reads `scene[name]` root/body state tensors, mount geometry, source schedules, directivity, quaternion order, and optional environment origins. It accepts only `float32` tensors already on the sensor device and supports `geometry_only` or `tdoa_synthetic` with effects disabled. R5.5 introduced reference binding over pure snapshot and array dataclasses; the v3 backend-contract cleanup replaces its duplicated array specifications with `bind_reference(snapshots, array_ids)`, so every scalar backend resolves the selected canonical array from its snapshot.
