...
Kit state, UI, maintained presets, headless summaries, validation, sensor construction, import, and export carry the explicit threshold. Subphase 03.3 introduced `ias.omni_extension_binding.v6` with exact required Auditok activity state and no v5 parser. Subphase 04.3 later supersedes it with binding v7 and no v6 parser while preserving the activity object unchanged. Export, JSONL, guided recording, Replicator, OmniGraph, and live instruments retain the resulting frame observations.
