...
The Kit build creates `PatrizioAcquadro-isaac-audio-sensors-linux-x86_64-v<version>.zip`. The explicit wheelhouse must match the hashes and exact six-distribution inventory in `tools/release/kit_dependencies.lock`: Auditok, `pyroomacoustics`, SciPy, SoundFile, CFFI, and pycparser. Acquisition verifies hashes while downloading; the preflight, builder, and post-build audit independently validate their input or artifact boundary.
