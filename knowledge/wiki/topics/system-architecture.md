...
R8.1 adds `AnalyticAcoustics` as a deterministic CPU L2 backend. It routes from the environment kind to Core direct or half-space propagation, or to lazy PyRoom shoebox or polygon-prism construction. R8.2 keeps direct and indirect pair stems internal, applies `SourceOcclusion` only to the direct stem, and exposes only the recombined multichannel waveform. The shared room pipeline owns scheduling, rendering, effects, detection, diagnostics, and frame assembly. The four legacy backend identifiers remain registered until R8.3 and downstream migration.
...
Its entity path converts official scene root/body pose tensors directly into batched fixed-shape observations. R8.2 keeps that path limited to `geometry_only` and `tdoa_synthetic`; `analytic_acoustics` mass-parallel execution belongs to R8.3. The separate reference path recognizes `analytic_acoustics`, converts pure core snapshots through scalar backends, and accepts only one array identifier per snapshot; it owns no parallel array specifications. USD discovery, stage poses, and environment anchoring remain in the Isaac Sim layer; Lab owns no stage adapter or device fallback.
