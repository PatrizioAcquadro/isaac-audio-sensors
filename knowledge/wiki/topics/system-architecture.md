...
R8.1 adds `AnalyticAcoustics` as a deterministic CPU L2 backend. It routes from the environment kind to Core direct or half-space propagation, or to lazy PyRoom shoebox or polygon-prism construction. The shared room pipeline still owns scheduling, rendering, effects, detection, diagnostics, and frame assembly. The four legacy backend identifiers remain registered during this subphase.
