...
The layer turns live USD state into pure core dataclasses before backend computation. `IsaacEnvironmentResolutionCfg` keeps simulator inputs separate from `AcousticEnvironmentSpec` and resolves manual environments, explicit anchors, or marked USD shoebox/half-space candidates before snapshot construction. The cache re-resolves on array motion or relevant marker, bounds, pose, and material changes. The sensor has no offline config path or application persistence: consumers inject an optional core waveform sink, and required Isaac APIs resolve lazily with explicit errors.
