...
`isaac_audio_sensors.core` owns typed scene, source, pose, array, acoustic-surface/environment, time-window, signal-block, observation, DOA, occlusion, and frame models; canonical directivity and gain utilities; configuration; microphone geometry; deterministic DSP and effects; acoustic backends; observed-only perception; plugins; calibration; trace IO; and waveform helpers. Its package root exports the fundamental models plus `DirectivityPattern`; environment builders and transforms are public from `core.acoustics`.
...
`AnalyticAcoustics` is the only registered runtime propagation backend. It routes from the environment kind to Core direct or half-space propagation, or to lazy PyRoom shoebox or polygon-prism construction. Direct and indirect pair stems remain internal, `SourceOcclusion` applies only to the direct stem, and the public signal block is the recombined multichannel result. Analytic internals own scheduling, rendering, effects, and signal diagnostics, but not observed perception. A temporary `simulate()` bridge still assembles zero-observation frames and waveform references until Plan 02.3 removes it. Removed geometry, synthetic-TDOA, and room backend behavior survives only as internal logic or historical v1 replay data, never as runtime aliases.

Motion owns Doppler and pose/window state; acoustics owns environment builders and transforms, materials, and occlusion interpretation; DOA owns the numerical least-squares solver as well as GCC-PHAT, SRP-PHAT, physical ambiguity, and sector mapping. Core returns compatible direction evidence and never applies contextual priors; consumers own assumptions such as a front hemisphere. Fundamental data contracts remain centralized in `core.types`.
...
The layer turns live USD state into pure core dataclasses before backend computation. `IsaacEnvironmentResolutionCfg` keeps simulator inputs separate from `AcousticEnvironmentSpec` and resolves manual environments, explicit anchors, or marked USD shoebox/half-space candidates before snapshot construction. The cache re-resolves on array motion or relevant marker, bounds, pose, material, and acoustic-partition changes. The sensor uses `analytic_acoustics`, forwards analytic solver options, and supplies minimal per-pair raycast attenuation through the live lifecycle. Isaac owns partition/material provenance and optional transient `DebugPrimitive` ray/hit traces; neither enters stable snapshots, frames, or datasets. The layer has no offline config path or application persistence: consumers inject an optional core waveform sink, and required Isaac APIs resolve lazily with explicit errors.
...
Its entity path resolves official scene root/body pose tensors and allocates the fixed-shape observation contract directly on the sensor device without per-environment loops. The separate reference path executes scalar Core snapshots with one array identifier per snapshot. Both paths deliberately return padded zero-observation tensors until Phase 03 supplies observed activity; neither derives activity, direction, confidence, ambiguity, or per-observation RMS from source state. USD discovery, stage poses, and environment anchoring remain in the Isaac Sim layer; Lab owns no stage adapter, contextual direction policy, or device fallback.
...
`ExtensionController` composes those services, owns the flat `ExtensionUiState`, reports status/errors, and exposes the maintained GUI/headless actions. `window.py` and `sections.py` only render state and invoke actions. Pure validation checks remain dependency-free; the stateful validation controller owns capability discovery, backend/device facts, calibration reads, and fail-closed environment-mode checks. The registry-derived backend choice is `analytic_acoustics`. Kit configuration uses `ias.omni_extension_binding.v5`; v2-v4 are rejected, no ambiguity-policy state is retained, `unconfigured` cannot validate or start, and maintained presets select explicit free field.
...
Configuration or live stage/entity state produces an authoritative `AudioSceneSnapshot`; an array identifier selects the observer and a propagation backend emits `MicrophoneSignalBlock`. `AudioPerceptionPipeline` combines that block with the exact array geometry and optional typed external observations to produce `AudioSensorFrame`; maintained default consumers currently use the temporary zero-observation bridge. Optional writers serialize traces or session shards, Isaac Lab projects the bounded observation contract into fixed tensors, and downstream projects adapt those outputs outside this repository.
