...
Its entity path resolves official scene root/body pose tensors and allocates the fixed-shape observation contract directly on the sensor device without per-environment loops. It deliberately returns padded zero-observation tensors, rejects `energy_threshold_dbfs`, and does not invent a signal block. The separate reference path requires that threshold, executes scalar Core snapshots with one array identifier and one persistent Auditok pipeline per environment, and resets only selected environments. Subphase 07.1 projects the reference path's actual `AudioObservation` sequences into finite tensors with validity, optional DOA, ambiguity, candidate masks, and truncation counts. Entity output remains empty pending 07.2; source state and per-source RMS never become observation tensors. The exact contract belongs to [[topics/isaac-lab-integration|Isaac Lab Integration]]. USD discovery, stage poses, and environment anchoring remain in the Isaac Sim layer; Lab owns no stage adapter, contextual direction policy, or device fallback.
