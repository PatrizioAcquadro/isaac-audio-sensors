...
`AudioPerceptionPipeline.process()` accepts a `MicrophoneSignalBlock`, its exact `MicrophoneArraySpec`, frame identity, and optional typed external observations. It validates array ID, sample rate, microphone order, and geometry; passes only valid channels to the injected `ActivityDetector.detect()` in their original order; and skips perception when no channel is valid. The detector object supplies its own stable identifier and returns `ActivityDecision`; there is no parallel `detector_id` pipeline argument. Inactive output emits no observation. Without an event localizer, active output emits one `signal_derived` observation. An injected `event_localizer` instead emits a variable event sequence, runs with at least two valid channels, and leaves individual detection scores absent; Auditok activity stays global. Aggregate RMS always comes from the observed block, never scene/source truth.

The optional `DoaEstimator` has exactly `estimate(samples, microphone_positions_m, sample_rate_hz) -> (DoaEstimate, diagnostics)`. Its samples are the read-only valid rows of the final combined block, and its matching XYZ geometry is array-local. The call has no scene, source count, source identity or position, schedule, private stem, or producer diagnostic. The registry contains only `tdoa_least_squares` and lazy optional `pyroomacoustics_srp`; internal `srp_phat` is removed. The retained single-estimate selector assigns least-squares to stereo and SRP to horizontal planar arrays. The standard pipeline now composes `MaintainedEventLocalizer`: at 16 kHz it uses WPE/group-sparse covariance for horizontal planar and rank-3 arrays, while stereo and other planar sample rates retain the single-event role. Other rank-3 rates are unavailable. The multisource implementation imports room dependencies and NARA-WPE lazily and is qualified only for the [[experiments/04-4-multisource-localization|bounded simulated indoor stable-source domain]].

Standard multisource DOA consumers retain exactly the latest 750 ms of causal valid mixture; stereo and other planar rates retain 250 ms, including inactive Auditok ticks, and estimate only on active ticks. The new event sequence emits no event before complete context and reports `perception.localization.status=unavailable` with `reason=insufficient_context`; complete empty results report `no_events`. Events sort deterministically by bearing/elevation/candidates and receive frame-local IDs without tracks. The original single-event role retains an unresolved warm-up estimate. On that single-event role, a resolved circular jump of at least 150 degrees abstains as `temporal_instability`; the new lobe requires a next-active-tick confirmation within 30 degrees. Inactivity clears stable and pending bearings but retains audio context. Explicit reset, layout/rate changes, stream identity changes, and non-contiguous windows clear all DOA state. Diagnostics expose one canonical causal context plus role, estimator, raw reliability, jump, pending/confirmed state, and abstention reason.

`AudioPerceptionPipeline.reset()` forwards reset to each injected stateful detector or estimator object exactly once by identity. Lifecycle owners still reset at episode and replay boundaries. One pipeline-owned continuity check now resets both plugins and all DOA context for declared faults, no valid channels, identity/clock/rate/layout changes, and gaps, overlaps, or backwards windows. A single `perception.reset_reason` records the first applicable cause. Timestamp comparison uses a small absolute/ULP tolerance, never a tolerance proportional to clock origin. Rigid array motion and provenance-only changes do not reset a contiguous signal. Missing channels retain their declared rows and validity masks; finite placeholder samples never enter perception.

Processing copies producer diagnostics and adds `signal` geometry, clock, discontinuity, clipping, and the unit amplitude reference beside `perception`. These namespaces survive ordinary frame recording/replay without a schema revision. Recorder `is_reset` and gap accounting remain caller/recorder-owned; pipeline resets do not invent recorder reset markers. No IO or calibration application occurs in perception.

Host processing duration, delivered-block-to-frame response, and delivery intervals belong in consumer reports, outside deterministic sensor frames. A common received-reference alignment permits comparison but does not establish physical emission time or absolute latency. [[implementation_phases/06-simulated-and-real-signal-parity|Subphase 06.3]] validates identical-sample observations and recording/replay across the maintained mono, stereo, and planar-array roles.
...
Import-safe protocols define propagation backends, activity detectors, DOA estimators, event localizers, and audio feature extractors. `EventLocalizer.localize(samples, microphone_positions_m, sample_rate_hz)` returns `(sequence[DoaEstimate], diagnostics)`; diagnostics distinguish `events`, `no_events` and `unavailable`. Its injection is mutually exclusive with `doa_estimator`. Ambiguous candidates within one estimate are not separate events; no serialized schema changes are needed. Internal fitted strengths are diagnostic only, with no individual probability; the maintained localizer leaves `bearing_confidence` at its default `None` and reports confidence availability as false. `ActivityDetector` requires `detector_id`, `detect(samples, sample_rate_hz) -> ActivityDecision`, and `reset()`; detector state owns temporal smoothing and event boundaries.

The unshipped streaming-localizer/orientation injection used by rejected temporal experiments was removed in 07.3. `EventLocalizer` remains the maintained mixture-only boundary; historical experiment results remain in [[experiments/04-4-multisource-localization|Multisource Localization]].
...
Applying relative geometry, gain, delay, polarity, response, confidence, or timing information requires values supported by the profile; absolute physical calibration and sim-to-real validity require external measurements and evidence. Downstream comparison retains original samples, nominal geometry, common plugin settings, independent resets, and raw endpoint clipping. The completed S4.5 gain-specific executable path is retired; its historical assessment remains evidence and admits no current correction. Correction admission and functional mapping verification remain separate from raw integration. [[implementation_phases/06-simulated-and-real-signal-parity|Plan 06]] records the physical reference case.
