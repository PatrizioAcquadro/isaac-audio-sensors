...
**Known availability defect, pending pre-07.2 correction:** the maintained multisource method declares confidence unavailable, yet its default zero receives a true confidence mask. The 2026-09-09 audit reproduced this on CUDA. The behavior above describes current code, not permission to interpret that placeholder as a measured score. [[implementation_phases/02-signal-and-perception-architecture|02.2]] owns the observed availability correction, and [[implementation_phases/07-isaac-lab-observation-integration|Phase 07]] owns its consumer integration.
