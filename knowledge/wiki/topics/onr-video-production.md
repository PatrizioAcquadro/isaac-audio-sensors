...
Run `.venv/bin/python local/onr/run.py <catalog|video1|video2> compose` to recompose saved recordings, followed by `verify`. For new acquisition, use `all --work build/onr/<new-run> --out evidence/<new-delivery>`; existing recordings are preserved. Source assets and essential recordings live outside `build/`, so `make clean` cannot delete the maintained generator or its inputs.
