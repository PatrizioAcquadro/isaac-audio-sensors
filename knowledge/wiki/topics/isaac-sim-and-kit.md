...
Import-safe world bounds and environment-absorption helpers live in `isaac_audio_sensors.isaac.usd_bounds`. `isaac_audio_sensors.isaac.environment_resolution` converts manual inputs, anchors, and marked USD candidates into the same `AcousticEnvironmentSpec` used by Core; the Isaac sensor and Kit share that resolver.
...
Lazy timeline time, update subscriptions, and reset subscriptions share one Isaac lifecycle helper. Environment resolution belongs to stage-snapshot helpers and is cached until array motion or relevant bounds, transform, marker, or acoustic-material changes invalidate it. Occlusion pair comparison, refresh reasons, and material diagnostics stay inside the occlusion module.

## Occlusion and Visualization

Optional PhysX all-hit segment queries compute per-source/per-microphone occlusion and nominal or authored whole-partition transmission. Solid-collider traversal and requested-but-unavailable capture are corrected; see [[implementation_phases/r8-analytic-acoustics-backend#Pre-07.2 Follow-up — Live Occlusion Correctness|the bounded live qualification]]. Injected raycasters now return complete hit sequences through `raycast_all`. Missing support stops capture; Kit clears current instruments and finalizes active guided recording as incomplete.

RMS instruments consume the complete observed microphone mixture. Nullable confidence displays `N/A`; measured zero remains zero. The compass shows simultaneous events in frame-local colors and unresolved bearing alternatives as dashed rays. It does not track identities or color events by simulation truth. Independent elevation alternatives appear in current-event details. A separate geometry label reports blocked/total direct paths. Frame-age warnings remain distinct from causal context and perceptual response delay. Provider diagnostics belong to [[implementation_phases/r10-geometry-acoustics-integration|08.3/R10.3]], and effect controls to [[implementation_phases/09-practical-realism-and-randomization|Phase 09]].
...
Only the current step is expanded. It exposes one primary action, with Back and recovery actions secondary. Setup applies a maintained safe preset with explicit `manual_free_field` plus stage bindings; Validate runs stage, backend, device, source, array, environment resolution, attachment, calibration, and capability checks; Run and Inspect reuse the same canonical sensor control as Live Monitor; Record exposes Start Recording, then Stop & Finalize, with Cancel separate; Export validates, splits when requested, and inventories the result. Capability validation refreshes stale cached facts within the same pass.
...
The GUI stores one complete current frame; metadata, mixture RMS and observed events derive from it. Current details show every retained event, activity, localization availability, sample rate, reported method/role, causal context and truncation. Missing diagnostics remain unavailable rather than being interpreted as silence. New GUI arrays default to 16 kHz; existing arrays, applied profiles and imported rates remain explicit. DOA activation is still optional. Stereo and other supported planar rates retain their separate single-event role.

Up to eight mixture RMS meters use a `-60 ... 0 dBFS` scale. History retains at most 50 observed events and shows its latest three entries, independently of the current frame. Repeated reads do not append duplicate events; an empty current frame clears its directions. Reset, reconfiguration, close and capture failure invalidate current data and episode history. Configuration import restores settings but never revives exported observations.
...
Numeric settings use drag widgets, including the required finite `Activity dBFS` threshold; DOA is a default-off Boolean control. Enumerated choices use combo boxes, and string fields remain limited to identifiers, paths, and free text. Saved configuration is validated before UI state or USD is mutated. Color styling distinguishes editable, action-populated, read-only, and invalid fields. All maintained controller capabilities remain reachable here without duplicating lifecycle controls that are simultaneously visible in Live Monitor.

Replicator controls the optional Omniverse writer; Export writes the latest frame, JSONL streams, and reusable `ias.omni_extension_binding.v7` configuration. Binding v7 requires exact Auditok `activity_detection` plus `direction_estimation: {"enabled": bool}` and retains the analytic solver, environment, and provenance sections. It serializes neither an estimator ID nor ambiguity policy. Binding v6 and older inputs are rejected without compatibility parsers.
...
If start/update fails, read the exact validation finding for backend, dependency, device, array geometry, source, environment mode/resolution, or output path instead of changing unrelated settings. Marked candidates require `ias:environment_kind` plus `ias:environment_id`; use an explicit anchor when automatic selection is ambiguous.
