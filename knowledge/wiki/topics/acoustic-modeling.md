...
`AnalyticAcoustics` owns `prepare -> render -> effects -> signal block`, including scheduling, Doppler, gain, directivity, and effects. Its temporary Plan 02.2 bridge can still write a waveform and assemble a zero-observation frame, but GCC/SRP estimation and source-conditioned observation construction are no longer backend responsibilities. Plan 02.3 removes that bridge.
