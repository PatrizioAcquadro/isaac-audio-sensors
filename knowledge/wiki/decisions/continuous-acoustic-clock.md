...
- Source/array orientation and microphone offsets use the current snapshot orientation. Rotating arrays and within-history orientation changes are not qualified.
...
Numerical propagation tests live in `tests/integration/test_propagation_continuity.py`. The independent motion/indoor DOA comparison and retained estimator decision are documented in [[experiments/04-4-multisource-localization|04.4 Multisource Localization]].
